...
class MinIOStorage:
    ...
    def from_env(cls) -> "MinIOStorage":
        ...
        - AWS_S3_SECURE (optional, defaults to False)
        ...
        """
        endpoint = _get_env_or_raise("AWS_S3_ENDPOINT")
        access_key = _get_env_or_raise("AWS_S3_ACCESS_KEY")
        secret_key = _get_env_or_raise("AWS_S3_SECRET_KEY")
        secure = os.getenv("AWS_S3_SECURE", "false").lower() in ("true", "1", "yes")
    ...
    async def save_recording(
        self, call_sid: str, audio_data: bytes, sample_rate: int, num_channels: int
    ) -> str:
    ...
    async def append_transcript(self, call_sid: str, line: str) -> str:
        ...
        object_name = f"{call_sid}/transcript_text.txt"

        # Read existing content (if any) in thread pool
        existing = ""
        try:
            response = await asyncio.to_thread(
                self.client.get_object, "transcripts", object_name
            )
    ...
    async def save_recording_from_chunks(
        self, call_sid: str, audio_chunks: list, sample_rate: int, num_channels: int
    ) -> str:
        ...
        return await self.save_recording(
            call_sid, audio_data, sample_rate, num_channels
        )

    async def save_transcript_from_lines(
        self, call_sid: str, transcript_lines: list
    ) -> str:
    ...
    async def get_object(self, bucket_name: str, object_name: str):
        ...
        return await asyncio.to_thread(self.client.get_object, bucket_name, object_name)
