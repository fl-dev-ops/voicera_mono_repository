...
async def memory_bootstrap(user_phone: str) -> Optional[Dict[str, Any]]:
    """Fetch all known facts + recent summaries for a user (pre-call).

    Returns: {facts: [{fact, created_at}], summaries: [{summary, call_id, created_at}]}
    """
    backend_url = _get_backend_url()
    api_endpoint = f"{backend_url}/api/v1/memory/bootstrap/{user_phone}"
    headers = _get_api_headers()
...
async def memory_extract_and_store(
    ...
    """Extract facts from an exchange and store new ones (non-blocking caller should use asyncio.create_task).

    Returns: {facts_extracted, facts_stored, facts_skipped}
    """
    backend_url = _get_backend_url()
    api_endpoint = f"{backend_url}/api/v1/memory/extract-and-store"
    headers = _get_api_headers()
...
async def memory_summarize(
    ...
    Returns: {summary, stored}
    """
    backend_url = _get_backend_url()
    api_endpoint = f"{backend_url}/api/v1/memory/summarize"
    headers = _get_api_headers()
