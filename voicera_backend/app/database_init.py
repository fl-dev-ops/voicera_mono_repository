...
def initialize_database():
    ...
    try:
        ...
        if "AgentConfig" not in existing_collections:
            ...
            agent_config.create_index(
                [("agent_type", 1), ("org_id", 1)],
                unique=True,
                name="agent_type_org_unique",
            )
            agent_config.create_index(
                [("agent_id", 1), ("org_id", 1)],
                unique=True,
                name="agent_id_org_unique",
            )
            agent_config.create_index("org_id", name="org_id_index")
            logger.info(
                "✓ Created AgentConfig with compound unique indexes on 'agent_type+org_id' and 'agent_id+org_id'"
            )
        else:
        ...
        if "Audience" not in existing_collections:
            ...
            audience.create_index(
                "audience_name", unique=True, name="audience_name_unique"
            )
        ...
        if "Campaigns" not in existing_collections:
            ...
            campaigns.create_index(
                "campaign_name", unique=True, name="campaign_name_unique"
            )
        ...
        if "CallLogs" not in existing_collections:
            ...
            call_logs = db["CallLogs"]
            try:
                call_logs.create_index(
                    "meeting_id", unique=True, name="meeting_id_unique"
                )
            except Exception as e:
                if (
                    "already exists" not in str(e).lower()
        ...
        if "PhoneNumber" not in existing_collections:
            ...
            phone_number.create_index(
                "phone_number", unique=True, name="phone_number_unique"
            )
            ...
            phone_number.create_index(
                [("org_id", 1), ("agent_type", 1)], name="org_agent_compound"
            )
        ...
        if "Members" not in existing_collections:
            ...
            members.create_index(
                [("email", 1), ("org_id", 1)], unique=True, name="email_org_unique"
            )
            members.create_index("org_id", name="org_id_index")
            members.create_index("email", name="email_index")
            logger.info(
                "✓ Created Members with compound unique index on 'email' + 'org_id'"
            )
        else:
        ...
        if "Integrations" not in existing_collections:
            ...
            integrations.create_index(
                [("org_id", 1), ("model", 1)], unique=True, name="org_model_unique"
            )
            integrations.create_index("org_id", name="org_id_index")
            logger.info(
                "✓ Created Integrations with compound unique index on 'org_id' + 'model'"
            )
        else:
        ...
        # 9. ScreeningResults collection
        if "ScreeningResults" not in existing_collections:
            logger.info("Creating ScreeningResults collection...")
            screening_results = db["ScreeningResults"]
            screening_results.create_index(
                "meeting_id", unique=True, name="meeting_id_unique"
            )
            screening_results.create_index("org_id", name="org_id_index")
            screening_results.create_index("outcome", name="outcome_index")
            screening_results.create_index(
                "candidate_phone", name="candidate_phone_index"
            )
            logger.info(
                "✓ Created ScreeningResults with indexes on meeting_id (unique), org_id, outcome, candidate_phone"
            )
        else:
            logger.debug(
                "ScreeningResults collection already exists. Ensuring indexes..."
            )
            screening_results = db["ScreeningResults"]
            try:
                screening_results.create_index(
        ...
        logger.info(
            "Collections verified: UserTable, AgentConfig, Audience, Campaigns, CallLogs, PhoneNumber, Members, Integrations, ScreeningResults"
        )
