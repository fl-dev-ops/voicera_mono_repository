...
def save_call_recording(recording_data: CallRecordingCreate) -> Dict[str, Any]:
    ...
    try:
        ...
        if recording_data.transcript_json_url:
            update_doc["transcript_json_url"] = recording_data.transcript_json_url
        ...
        updated_meeting = meeting_table.find_one(
            {"meeting_id": recording_data.call_sid}
        )
        ...
        if updated_meeting:
            ...
            logger.warning(
                f"Call recording saved but document not found: {recording_data.call_sid}"
            )
            return {
                "status": "fail",
                "message": "Recording saved but document not found",
            }
